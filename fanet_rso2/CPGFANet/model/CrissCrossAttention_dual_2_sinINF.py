...
class FEANet(nn.Module):
    ...
    def __init__(self, n_class, pretrained_backbone=True):
        ...
        def build_resnet(factory, weights_enum_name):
            if pretrained_backbone:
                try:
                    weights_enum = getattr(models, weights_enum_name)
                    return factory(weights=weights_enum.IMAGENET1K_V1)
        ...
        if self.num_resnet_layers == 50:
            resnet_raw_model1 = build_resnet(models.resnet50, 'ResNet50_Weights')
            resnet_raw_model2 = build_resnet(models.resnet50, 'ResNet50_Weights')
            self.inplanes = 2048
        elif self.num_resnet_layers == 101:
            resnet_raw_model1 = build_resnet(models.resnet101, 'ResNet101_Weights')
            resnet_raw_model2 = build_resnet(models.resnet101, 'ResNet101_Weights')
            self.inplanes = 2048
        elif self.num_resnet_layers == 152:
            resnet_raw_model1 = build_resnet(models.resnet152, 'ResNet152_Weights')
            resnet_raw_model2 = build_resnet(models.resnet152, 'ResNet152_Weights')
            self.inplanes = 2048
