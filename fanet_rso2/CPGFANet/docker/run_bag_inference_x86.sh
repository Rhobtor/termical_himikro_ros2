...
OUTPUT_DIR_DEFAULT="${REPO_ROOT}/ros2_outputs/bag_inference_docker"

IMAGE_NAME=${IMAGE_NAME:-cpgfanet-x86-cpu:latest}
...
if [[ "${BUILD_IMAGE}" == "1" ]]; then
  echo "[build] Construyendo imagen Docker ${IMAGE_NAME}"
  docker build -f "${REPO_ROOT}/docker/x86_cpu.Dockerfile" -t "${IMAGE_NAME}" "${REPO_ROOT}"
fi

echo "[run] Lanzando inferencia de rosbag con display X11"
echo "[run] Bag: ${BAG_PATH}"
echo "[run] Output: ${OUTPUT_DIR}"
echo "[run] Pares: ${MAX_PAIRS} stride=${SAMPLE_STRIDE} wait_ms=${DISPLAY_WAIT_MS}"
...
docker run --rm -it \
  ...
  --name cpgfanet_bag_inference_x86 \
  "${IMAGE_NAME}" \
